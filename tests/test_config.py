...
def test_sandbox_open_allows_root(monkeypatch) -> None:
    _base_env(monkeypatch)
    monkeypatch.setenv("SANDBOX_OPEN", "true")
    monkeypatch.setenv("SELF_DEPLOY_ENABLED", "false")
    clear_settings_cache()
    settings = Settings(_env_file=None)
    assert settings.allowed_project_roots == [Path("/")]
    assert settings.resolved_agent_workspace == Path("/")


def test_self_deploy_extends_allowed_prefixes(monkeypatch) -> None:
    _base_env(monkeypatch)
    monkeypatch.setenv("SELF_DEPLOY_ENABLED", "true")
    clear_settings_cache()
    settings = Settings(_env_file=None)
    assert "systemctl" in settings.effective_allowed_command_prefixes
    assert "docker" in settings.effective_allowed_command_prefixes
